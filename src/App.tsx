...
const Display = styled.div<DisplayStyledProps>`
  width: 100%;
  margin-top: 50px;
  height: 65px;
  ...
  const handleEquals = useCallback(
    (label: string) => {
      ...
      if (label === "+/-") {
        ...
        setHistory((history) =>
          history.concat(`${firstFactor}+/-=${newState}`)
        );
      } else if (label === "%") {
        ...
        setHistory((history) =>
          history.concat(
            `${firstFactor}${currentOperation}${secondFactor}=${newState}`
          )
        );
      } else if (currentOperation === "+") {
        newState = String(firstFactor + secondFactor);
  ...
  return (
    <Container>
      {show ? (
        ...
        <ShowHistoryBtn onClick={() => setShow(true)}>History</ShowHistoryBtn>
      )}
      <Display displayLength={Number(display).toLocaleString().length}>
